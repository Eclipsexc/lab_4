...
export async function uploadInvestors() {
  try {
    const ref = collection(db, "investors");
    for (const investor of investors) {
      await addDoc(ref, investor);
    }
    console.log("✅ Інвестори успішно додані до бази!");
  } catch (error) {
    console.error("❌ Помилка під час додавання інвесторів:", error);
  }
}
